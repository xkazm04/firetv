...
When Linga knows neither a goal nor an interest — a learner who picked their own level never met the check's questions — it asks first: *What would you like to practise in English?* The answer (any language, spoken or typed) becomes the learner's goal, so the next plan does not ask again; *Let Linga pick* skips it. Topics cut with nothing to go on came out generic in the second UAT run.
...
A moment is two quotes and a caption, with one action: back to the conversation, at the same beat. The partner's reply waits and plays after it. The coaching preference now reads *Stop me to fix a mistake or teach a word* (the new default) or *Only coach me when I ask*, which turns moments off. At most one moment in any three learner turns and four in a rehearsal; at A1–A2, one in any four and two in a rehearsal, and only for something the learner will need again in the scene. A *word* moment is vocabulary only — a grammar point is a fix, which must quote the learner's English — and on a reply in another language it reads *You wanted to say* / *In English*. A moment models wording, so the reply after it is supported practice. What was taught is kept on the learner record (last 60), listed in the recap on the phone and under *Things Linga taught you* on the map, ready for word practice.
...
**Taught phrases come back (25 Sep 2026).** What a moment taught used to be read in one place, a collapsed list on the phone. Now `lib/english/review.ts` brings one item back into a later scene. At the start, code picks the item that is due: from an earlier rehearsal, never used again, offered fewer than two times, the oldest first. The scene records it as `Conversation.review` and counts the offer on the record (`Taught.offered`). While the item is unused, the opening, turn and replay prompts carry `bringBack` (the phrase, what it means, and "invite it, never say it"); the coach's prompt does not. The model only makes room for the phrase. Code decides whether it was used: normalised word-for-word containment in an unsupported reply, never after this scene already showed the phrase (a partner line, the cue, a moment, the coach's alternative), and only once. A hit writes `reusedAt`, `reusedIn` and `reusedQuote` on the taught item and `review.used` on the scene. Model evidence and progress are untouched, and nothing is scored (AG-2). Withholding holds: during the scene the phrase is on no screen until the learner has said it. The line after that reply names it once under *Used again*. On the TV recap, a phrase that was used is the before-and-after picture: *Linga taught · <earlier scene>* beside *You said it tonight*, with the star-burst in the arch. A phrase that was not used is the recap's sentence to take with you. The phone marks it *used again* in the recap and on the map.
...
- `lib/english/conversation.ts`: band-pitched tutor prompt, the check's focus in the context, scene copies, moments, and the review wiring: `bringBack` in the context, the offer at start, `reuseOf` on each turn.
- `lib/english/review.ts`: `dueTaught`, `bringBack`, `reuseOf` (normalised containment), `offer`, `markReused`.
