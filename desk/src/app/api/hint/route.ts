...
export async function POST(req: Request) {
  ...
  if (body.stage === 2 && prev && prev.key === item.key) {
    const r = await runJob("hint", async () => {
      ...
      return h2;
    }, { key: item.key, start: "thinking one step further…", done: (h2) => `second hint in ${(h2.ms / 1000).toFixed(1)} s` });
  ...
  const r = await runJob("hint", async () => {
    ...
    return h1;
  }, { key: item.key, start: "thinking about a hint…", done: (h1) => `hint in ${(h1.ms / 1000).toFixed(1)} s · finding the lesson…` });
