...
const turnAccept=schema({reply:str(230),supportProvided:{type:"boolean"},observations:{type:"array"},moment:{type:"object"}});
...
export async function englishCommand(raw:unknown){
  ...
  try{
    ...
    const result=await text<Record<string,unknown>>({system:tutorSystem(c),prompt:JSON.stringify({...context(c),...task}),schema:action==="turn"?turnSchema:action==="coach"?coachSchema:replaySchema,accept:action==="turn"?turnAccept:undefined,model:"fast",timeoutMs:60000,isolated:true});
